...
function validateRow() {
  ...
  if (!dayjs(newStart.value).isValid() || !dayjs(newEnd.value).isValid()) return
  if (dayjs(newStart.value) > dayjs(newEnd.value)) return
  addRow()
}
              ...
              <tr>
                ...
                <td class="pds-pr-0">
                  <div class="pds-flex pds-gap-xs date">
                    <div class="pds-flex pds-gap-xs faux-input faux-input-start">
                      ...
                      <input type="text" onBlur=${(event) => updateValue('start', index)} value=${parseForever(item.start)} class="pds-flex-fill" />
                    </div>
                    to
                  </div>
                </td>
                <td class="pds-pl-xs">
                  <div class="pds-flex pds-gap-xs date faux-input faux-input-end">
                    ...
                    <input type="text" onBlur=${(event) => updateValue('end', index)} value=${parseForever(item.end)} class="pds-flex-fill" />
                  </div>
                </td>
                <td>
